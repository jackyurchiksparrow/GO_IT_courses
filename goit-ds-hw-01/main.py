...
class Name(Field):
    ...
    def __init__(self, name):
        # call the parent constructor to save the attribute value
        super().__init__(name)
...
class Phone(Field):
    ...
    def __init__(self, phone_num: str, dig_qty: int = 10):
        dig_sum = 0
        for ch in phone_num:
            if ch.isdigit():
                dig_sum += 1
            else:  # we also forbid "+" because 10 digits means no country code
                raise ValueError("Phone number may only numbers and exactly 10 of them")
...
class Birthday(Field):
    def __init__(self, value: str):
        try:
            birthday_dt = datetime.strptime(value, "%d.%m.%Y")  # noqa: DTZ007
...
class Record:
    ...
    def edit_phone(self, old_phone_num: str, new_phone_num: str):
        ...
        if phone_obj:
            ...
            msg = f"Number doesn't exist: {old_phone_num}"
...
class AddressBook(UserDict):
    ...
    def get_upcoming_birthdays(self, days=7):
        ...
        for username, usr_data in self.data.items():
            ...
            bday = _string_to_date(usr_data.birthday.value)
